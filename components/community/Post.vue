...
const formattedCreatedAt = computed(() => formatTimeElapsed(props.post.createdAt))
...
const isLiked = computed(() => props.post.likes.some(like => like.userId === userStore.user?.id))
...
const like = async () => {
  ...
  try {
    ...
    if (message === 'Like') {
      ...
      props.post.likes = props.post.likes.filter(like => like.userId !== payload.userId)
    }
  } catch (error) {
...
const onDeleteComments = (id: string) => props.post.comments = props.post.comments.filter(comment => comment.id !== id)
