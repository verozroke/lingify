<template>
  <div class="pl-20 border-l-2 border-slate-200 my-5 w-full bg-white ">
    <div class="flex w-full items-center justify-between">
      <div class="flex items-center space-x-3">
        <div
          class="h-5 w-5 rounded-full bg-center bg-cover"
          :style="{ backgroundImage: comment.owner.avatar ? `url('${comment.owner.avatar.url}')` : 'url(data:image/png;base64,)' }"
        >
        </div>
        <div class="text-base font-bold text-slate-700">{{ comment.owner.nickname }}</div>
      </div>
      <div class="flex items-center space-x-8">
        <div class="text-xs text-slate-500">{{ formattedCreatedAt }}</div>
      </div>
    </div>
    <div class="mt-2">
      <div class="text-sm text-slate-600">{{ comment.text }}</div>
    </div>
    <div class="flex mt-2 items-center justify-end">
      <UiButton
        @click="reply"
        mode="text"
        density="compact"
        color="#475569"
        text-color="#475569"
        prepend-icon="mdi-reply"
        :loading="isLoading"
        :disabled="isLoading"
      >Reply</UiButton>
      <UiButton
        v-if="comment.owner.id === userStore.user?.id || userStore.user?.id === communityStore.community.owner.id"
        @click="deleteComment"
        mode="text"
        density="compact"
        color="red"
        prepend-icon="mdi-delete"
        :loading="isLoading"
        :disabled="isLoading"
      >
        Delete
      </UiButton>
    </div>
  </div>
</template>

<script setup lang="ts">
import type { Comment } from '~/core/types/community'
import { useToast } from '~/hooks/use-toast';
import postService from '~/services/post.service';

const userStore = useUserStore()
const communityStore = useCommunityStore()
const postStore = usePostStore()
const { toast } = useToast()

const props = defineProps<{
  comment: Comment
  onDeleteComment: (id: string) => void
}>()

const formattedCreatedAt = computed(() => formatTimeElapsed(props.comment.createdAt))

const reply = () => {
  postStore.commentInput = `@${props.comment.owner.nickname}, `
}

const isLoading = ref(false)

const deleteComment = async () => {
  try {
    isLoading.value = true
    await postService.deleteComment(props.comment.id)

    props.onDeleteComment(props.comment.id)
    isLoading.value = false
    toast.success({ message: 'Comment was deleted successfully.' })
  } catch (error) {
    isLoading.value = false
    toast.error({ message: 'Could not delete your comment.' })
  }
}
</script>

<style scoped></style>