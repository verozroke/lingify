<template>
  <div class="flex flex-col pb-4 w-full">
    <div class="text-2xl font-bold w-full leading-10 text-oxford">Настройки учетной записи</div>
    <div class="text-sm font-medium w-full leading-5 text-slate-500">Lorem ipsum dolor sit amet consectetur,
      adipisicing
      elit. Commodi maxime deserunt doloremque unde enim quaerat!</div>
    <div class="flex bg-slate-300 my-4 h-[1px] rounded-full w-full"></div>
    <div class="flex gap-10 mt-10">
      <div class="h-28 w-28 rounded-full">
        <img
          class="h-28 w-28 object-cover rounded-full"
          :src="userStore.user?.avatar?.url ? userStore.user.avatar.url : 'data:image/png;base64,'"
          alt="User Avatar"
        />
      </div>
      <div class="flex flex-col">
        <div class="text-xl font-bold w-full leading-10 text-oxford">Ваш аватар</div>
        <div class="text-xs font-medium w-full leading-5 -mt-2 text-slate-500">Lorem ipsum dolor sit amet consectetur
          adipisicing elit.</div>
        <v-text-field
          :rules="imageURLRules"
          placeholder="Вставьте URL Аватарки."
          clearable
          label="URL Аватарки"
          v-model="settingStore.imageUrlInput"
          class="mt-5"
          prepend-inner-icon="mdi-link-variant"
          variant="outlined"
        ></v-text-field>
        <v-btn
          @click="async () => await settingStore.saveImage()"
          style="width: fit-content; text-transform: none;"
          class="px-4 mt-2 text-white"
          color="#28B0A6"
        >Сохранить</v-btn>
      </div>
    </div>
    <div class="flex gap-10 mt-10">
      <div class="h-28 w-28 rounded-full">
        <img
          class="h-28 w-28 object-cover rounded-full"
          :src="userStore.user?.banner?.url ? userStore.user.banner.url : 'data:image/png;base64,'"
          alt="User Avatar"
        />
      </div>
      <div class="flex flex-col">
        <div class="text-xl font-bold w-full leading-10 text-oxford">Ваш баннер</div>
        <div class="text-xs font-medium w-full leading-5 -mt-2 text-slate-500">Lorem ipsum dolor sit amet consectetur
          adipisicing elit.</div>
        <v-text-field
          :rules="imageURLRules"
          placeholder="Вставьте URL Баннера."
          clearable
          label="URL Баннера"
          v-model="settingStore.bannerUrlInput"
          class="mt-5"
          prepend-inner-icon="mdi-link-variant"
          variant="outlined"
        ></v-text-field>
        <v-btn
          @click="async () => await settingStore.saveBanner()"
          style="width: fit-content; text-transform: none;"
          class="px-4 mt-2 text-white"
          color="#28B0A6"
        >Сохранить</v-btn>
      </div>
    </div>
    <div class="flex flex-wrap gap-10 mt-10">
      <div class="flex flex-col">
        <div class="text-xl font-bold w-full leading-10 text-oxford">Никнейм</div>
        <div class="text-xs font-medium w-full leading-5 -mt-2 text-slate-500">Lorem ipsum dolor sit amet consectetur
          adipisicing elit.</div>
        <v-text-field
          :rules="nameRules"
          placeholder="Напишите никнейм"
          clearable
          label="Никнейм"
          v-model="settingStore.nameInput"
          class="mt-5"
          prepend-inner-icon="mdi-account-edit"
          variant="outlined"
        ></v-text-field>
        <v-btn
          @click="async () => await settingStore.saveUsername()"
          style="width: fit-content; text-transform: none;"
          class="px-4 mt-2 text-white"
          color="#28B0A6"
        >Сохранить</v-btn>
      </div>
      <div class="flex flex-col">
        <div class="text-xl font-bold w-full leading-10 text-oxford">Email</div>
        <div class="text-xs font-medium w-full leading-5 -mt-2 text-slate-500">Lorem ipsum dolor sit amet consectetur
          adipisicing elit.</div>
        <v-text-field
          :rules="emailRules"
          placeholder="Напишите ваш email"
          clearable
          label="Email"
          v-model="settingStore.emailInput"
          class="mt-5"
          prepend-inner-icon="mdi-at"
          variant="outlined"
        ></v-text-field>
        <v-btn
          @click="async () => await settingStore.saveEmail()"
          style="width: fit-content; text-transform: none;"
          class="px-4 mt-2 text-white"
          color="#28B0A6"
        >Сохранить</v-btn>
      </div>
      <div class="flex flex-col">
        <div class="text-xl font-bold w-full leading-10 text-oxford">Полное имя</div>
        <div class="text-xs font-medium w-full leading-5 -mt-2 text-slate-500">Lorem ipsum dolor sit amet consectetur
          adipisicing elit.</div>
        <v-text-field
          :rules="fullNameRules"
          placeholder="Напишите ваше полное имя"
          clearable
          label="Полное имя"
          v-model="settingStore.fullNameInput"
          class="mt-5"
          prepend-inner-icon="mdi-account-edit"
          variant="outlined"
        ></v-text-field>
        <v-btn
          @click="async () => await settingStore.saveFullName()"
          style="width: fit-content; text-transform: none;"
          class="px-4 mt-2 text-white"
          color="#28B0A6"
        >Сохранить</v-btn>
      </div>
      <div class="flex flex-col">
        <div class="text-xl font-bold w-full leading-10 text-oxford">Био</div>
        <div class="text-xs font-medium w-full leading-5 -mt-2 text-slate-500">Lorem ipsum dolor sit amet consectetur
          adipisicing elit.</div>
        <v-text-field
          placeholder="Напишите о себе"
          clearable
          label="Био"
          v-model="settingStore.bioInput"
          class="mt-5"
          prepend-inner-icon="mdi-text-account"
          variant="outlined"
        ></v-text-field>
        <v-btn
          @click="async () => await settingStore.saveBio()"
          style="width: fit-content; text-transform: none;"
          class="px-4 mt-2 text-white"
          color="#28B0A6"
        >Сохранить</v-btn>
      </div>
      <div class="flex flex-col">
        <div class="text-xl font-bold w-full leading-10 text-oxford">Био</div>
        <div class="text-xs font-medium w-full leading-5 -mt-2 text-slate-500">Lorem ipsum dolor sit amet consectetur
          adipisicing elit.</div>
        <v-select
          label="Страна"
          v-model="settingStore.countryInput"
          :items="countries"
          variant="outlined"
          class="mt-5"
          prepend-inner-icon="mdi-map-marker"
        >
        </v-select>
        <v-btn
          @click="async () => await settingStore.saveCountry()"
          style="width: fit-content; text-transform: none;"
          class="px-4 mt-2 text-white"
          color="#28B0A6"
        >Сохранить</v-btn>
      </div>
    </div>
    <div class="text-2xl font-bold w-full leading-10 text-oxford mt-10">Смена пароля</div>
    <div class="text-sm font-medium w-full leading-5 text-slate-500">Lorem ipsum dolor sit amet consectetur,
      adipisicing
      elit. Commodi maxime deserunt doloremque unde enim quaerat!</div>
    <div class="flex bg-slate-300 my-4 h-[1px] rounded-full w-full"></div>
    <div class="flex flex-wrap gap-10">
      <div class="flex flex-col">
        <div class="text-xl font-bold w-full leading-10 text-oxford">Старый пароль</div>
        <div class="text-xs font-medium w-full leading-5 -mt-2 text-slate-500">Lorem ipsum dolor sit amet consectetur
          adipisicing elit.</div>
        <v-text-field
          :counter="20"
          :rules="passwordRules"
          density="compact"
          style="width: 350px;"
          placeholder="Напишите ваш старый пароль"
          type="password"
          clearable
          label="Старый пароль"
          v-model="settingStore.oldPasswordInput"
          class="mt-5 w-full"
          prepend-inner-icon="mdi-lock-question"
          variant="outlined"
        ></v-text-field>
      </div>
      <div class="flex flex-col">
        <div class="text-xl font-bold w-full leading-10 text-oxford">Новый пароль</div>
        <div class="text-xs font-medium w-full leading-5 -mt-2 text-slate-500">Lorem ipsum dolor sit amet consectetur
          adipisicing elit.</div>
        <v-text-field
          :counter="20"
          :rules="passwordRules"
          density="compact"
          style="width: 350px;"
          placeholder="Напишите ваш новый пароль"
          type="password"
          clearable
          label="Новый пароль"
          v-model="settingStore.newPasswordInput"
          class="mt-5 w-full"
          prepend-inner-icon="mdi-lock"
          variant="outlined"
        ></v-text-field>
      </div>
      <div class="flex flex-col">
        <div class="text-xl font-bold w-full leading-10 text-oxford">Подтверждение пароля</div>
        <div class="text-xs font-medium w-full leading-5 -mt-2 text-slate-500">Lorem ipsum dolor sit amet consectetur
          adipisicing elit.</div>
        <v-text-field
          :counter="20"
          :rules="passwordRules"
          density="compact"
          style="width: 350px;"
          placeholder="Повторите ваш новый пароль"
          type="password"
          clearable
          label="Подтверждение пароля"
          v-model="settingStore.newPasswordRepeatInput"
          class="mt-5 w-full"
          prepend-inner-icon="mdi-lock-check"
          variant="outlined"
        ></v-text-field>
      </div>
    </div>
    <v-btn
      @click="async () => await settingStore.savePassword()"
      style="width: fit-content; text-transform: none;"
      class="px-4 mt-2 text-white"
      color="#28B0A6"
    >Сменить пароль</v-btn>
  </div>
</template>

<script setup lang="ts">
import type { Country } from '~/core/types/country';
import { useToast } from '~/hooks/use-toast';
import countryService from '~/services/country.service';
import { useSettingStore } from '~/stores/SettingStore';
import { useUserStore } from '~/stores/UserStore';
import { isEmail, isValidUrl } from '~/utils/utils';

const userStore = useUserStore()
const settingStore = useSettingStore()
const { toast } = useToast()

const nameRules = ref([
  (v: any) => !!v || 'Name is required',
])

const emailRules = ref([
  (v: any) => !!v || 'Email is required',
  (v: any) => isEmail(v) || 'Not valid email',
])
const imageURLRules = ref([
  (v: any) => !!v || 'Image URL is required',
  (v: any) => isValidUrl(v) || 'Must be URL',
])

const fullNameRules = ref([
  (v: any) => !!v || 'Full name is required',

])

const passwordRules = ref([
  (v: any) => !!v || 'Password is required',
  (v: any) => (v && v.length >= 8) || 'Password must be more than 8 characters',
  (v: any) => (v && v.length <= 20) || 'Password must be less than 20 characters',
])

const countries = ref<string[]>([])

const fetchCountries = async () => {
  try {
    const fetchedContries: Country[] = await countryService.getCountries()
    countries.value = fetchedContries.map(country => country.name)
  } catch (error) {
    toast.success({ message: 'Could not fetch countries.' })
  }
}

onMounted(() => fetchCountries())

</script>

<style scoped></style>