    ...
    >
      ...
      <v-list class="mr-1 mt-10 py-0">
        <div class="flex flex-col  py-4 pl-6 pr-10">
          <span class="text-oxford text-lg font-semibold">{{ userStore.user?.nickname }}</span>
        ...
        <v-list-item
          ...
          value="Signout"
        >
          <v-list-item-title class="flex text-seagreen items-center gap-2 py-2 pl-1 pr-10">
...
const signOut = async () => {
  try {
    ...
    isLoading.value = false
    toast.error({ message: 'Could not logged out ' })
  }
}
