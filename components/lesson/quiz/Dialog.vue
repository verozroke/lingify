...
import type {
  GetQuestionsPayload,
  Question,
} from "~/services/chat-gpt.service";
...
const fetchQuiz = async () => {
  try {
    ...
    const fetchedQuestions: Question[] = await chatGPTService.getQuestions(
...
const checkQuiz = async () => {
  ...
  const correctAnswerLength = questions.value.reduce(
    (acc, question) =>
      acc + (question.correctAnswer === question.selectedVariant ? 1 : 0),
    0,
  );
