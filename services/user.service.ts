...
import type { ChangeAvatarValidator, ChangeBioValidator, ChangeCountryValidator, ChangeEmailValidator, ChangeFullNameValidator, ChangeNameValidator, ChangePasswordValidator } from "~/stores/SettingStore";
...
class UserService {
  ...
  async changeBanner(userId: string, body: ChangeBannerValidator) {
    const { data } = await axios.patch<MessageResponse>(`${this.baseUrl}/users/${userId}/banner`, body, {
      withCredentials: true,
      headers: {
        'Content-Type': 'application/json'
      }
  ...
  async changeCountry(userId: string, body: ChangeCountryValidator) {
    const { data } = await axios.patch<Country>(`${this.baseUrl}/users/${userId}/country`, body, {
      withCredentials: true,
      headers: {
        'Content-Type': 'application/json'
      }
  ...
  async changeBio(userId: string, body: ChangeBioValidator) {
    const { data } = await axios.patch<MessageResponse>(`${this.baseUrl}/users/${userId}/bio`, body, {
      withCredentials: true,
      headers: {
        'Content-Type': 'application/json'
      }
  ...
  async changeFullName(userId: string, body: ChangeFullNameValidator) {
    const { data } = await axios.patch<MessageResponse>(`${this.baseUrl}/users/${userId}/full-name`, body, {
      withCredentials: true,
      headers: {
        'Content-Type': 'application/json'
      }
