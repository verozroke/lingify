  ...
  <main class="profile-page">
    <section class="relative block h-500-px">
      <div
        ...
        :style="{
          'background-image': userStore.user?.banner ? `url(${userStore.user?.banner})` : 'none',
        }"
      >
        <NuxtLink to="/settings">
          <span
            ...
            v-ripple
            :style="{
              backgroundColor: !userStore.user?.banner ? colors.SEAGREEN : 'none'
            }"
            class="text-white w-full h-full absolute"
          ></span>
        </NuxtLink>
      </div>
      <div
          ...
          <div class="px-6">
            ...
            <div class="text-center mt-12">
              <h3 class="text-4xl font-semibold leading-normal mb-2 text-blueGray-700 mb-2">
                {{ userStore.user?.nickname ? userStore.user.nickname : 'Your nickname' }}
              </h3>
              <div class="text-sm leading-normal mt-0 mb-2 text-blueGray-400 font-bold">
                <v-icon>mdi-account</v-icon>
                {{ userStore.user?.fullName ? userStore.user.fullName : 'Your full name' }}
              </div>
              <div class="text-sm leading-normal mt-0 mb-2 text-blueGray-400 font-bold">
                <v-icon>mdi-map-marker-radius</v-icon>
                {{ userStore.user?.country ? userStore.user.country : 'Your country' }}
              </div>
              ...
              <div class="flex flex-wrap justify-center">
                <div class="w-full lg:w-9/12 px-4">
                  <p class="mb-4 text-base leading-relaxed text-blueGray-500">
                    {{ userStore.user?.bio ? userStore.user.bio : 'Здесь вы расскажите о себе.' }}
                  </p>
                </div>
              </div>
            </div>
