...
export const useUserStore = defineStore('userStore', () => {
  ...
  const logout = async () => {
    try {
      ...
      toastStore.openSnackbar('Выход из вашего аккаунта произведен успешно.', 2000, colorsEnum.EMERALD)
      isLogoutLoading.value = false
      isAuthenticated.value = false
      router.push('/login')
    } catch (error) {
      toastStore.openSnackbar('Не удалось выйти из вашего аккаунта.', 2000, colorsEnum.RED)
      isLogoutLoading.value = false
    }
  }
